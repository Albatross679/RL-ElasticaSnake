...
def evaluate_gait(env, action_array):
    ...
    for _ in range(100):
        ...
        # (Note: env expects raw actions [-1, 1], not pre-scaled torques)
...
def main():
    ...
    # Start with the known Forward Gait (converted to action space -1 to 1)
    # Assuming the forward gait corresponds to roughly max torque in specific patterns
    best_action = np.array([0.5, 0.5, 0.5, 0.5, 0.5, 0.5]) 
    best_score = -9999
    ...
    for i in range(total_iterations):
        ...
        noise = np.random.normal(0, 0.5, size=6)
        candidate_action = np.clip(best_action + noise, -1.0, 1.0)
